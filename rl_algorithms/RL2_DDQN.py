...
class DDQNAgent:
    def __init__(self):
        ...
        if self.ddqn:
            print("----------Double DQN--------")
            self.Model_name = os.path.join(self.Save_Path,"DDQN_"+self.env.env_name+".h5")
        else:
            print("-------------DQN------------")
            self.Model_name = os.path.join(self.Save_Path,"DQN_"+self.env.env_name+".h5")


        # create the controller model
        self.controller = controller(input_shape=(self.state_size,), action_space = self.action_size)
        self.target_controller = controller(input_shape=(self.state_size,), action_space = self.action_size)
    ...
    def PlotModel(self, score, episode):
        ...
        if self.Soft_Update:
            softupdate = '_soft'
        try:
            pylab.savefig(dqn+self.env.env_name+softupdate+".png")
        except OSError:
            pass
