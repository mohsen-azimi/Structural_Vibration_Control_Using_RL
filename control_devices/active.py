...
#         z2 = self.z[i]
#         volt = action
#         v1 = self.vel[i-1]*0.001  # (unit=m)
#         v2 = self.vel[i] * 0.001  # (unit=m)
#         eta = self.eta
#
#         gamma = self.gamma
#         beta = self.beta
...
#         self.force[i] = force = alpha * z3 + c0 * v2
#         # self.force[i] = max(min(alpha * z3 + c0 * v2, self.Fmax), -self.Fmax)
#
#         self.u[i] = u
#         if i >= GM.resampled_npts-1:
#             self.z[i] = z3
#         else:
#             self.z[i + 1] = z3
#         # self.dz[i] = dz
...
#     GM = LoadGM(dt=.01, t_final=50, g=9810., SF=0.5, inputFile='RSN1086_NORTHR_SYL090.AT2', outputFile='myEQ.dat', plot=False)
#
